 ...
 * Hack by Manolis Nikiforakis
 *
 * DESCRIPTION
 * The EthernetGateway sends data received from sensors to the WiFi link.
 * The gateway also accepts input on ethernet interface, which is then sent out to the radio network.
 *
 *  In addition, an OLED screen and a DHT11 are used, and sensor values can be directly forwarded to a cloud service (e.g. to Thingspeak) without using a controller.
 ...
 * This code has been tested on NodeMCU dev kit v1.0 .
 * The ESP8266 requires a certain pin configuration to download code, and another one to run code:
 * - Connect OLED SCL to D4
 * - Connect OLED SDA to D3
 * - Connect DHT11 Data to D0
 * - nRF is connect as described in mysensors
 *
...
const char* thingspeak_key = "your key";
...
void setup()
{
  ...
  sendStrXY("EXT:", 2, 0); 
  sendStrXY("INT:", 4, 0);   
  sendStrXY("INTfeels:", 6, 0);   
  char buffer[25];
  sendStrXY( floatToString(buffer,t,1), 4, 4);   
  sendStrXY( floatToString(buffer,h,0), 4, 10);   
  sendStrXY( floatToString(buffer,hic,1), 6, 10);   
...
void incomingMessage(const MyMessage &message) {
  ...
  if (message.type == V_TEMP) {
    sendStrXY(message.getString(convBuf), 2,4);
    thingspeak(message.getString(convBuf));
  }
